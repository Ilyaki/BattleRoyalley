...
namespace BattleRoyale
{
    class ModEntry : Mod, IAssetLoader
    {
        ...
        public ModEntry()
        {
            ...
            //if (Harmony.GetAllPatchedMethods().Count() > 0)
            //{
            //    Game1.quit = true;
            ...
            //{
            //    var orig_method = typeof(PatchProcessor).GetMethod("Patch");
            //    var new_method = new HarmonyMethod(GetType().GetMethod(nameof(PPPP)));
            //    h.Patch(orig_method, prefix: new_method);
            //}
        }
        ...
        public static bool PPPP(MethodBase original)
        {
            ...
            types.AddRange(Assembly.GetAssembly(typeof(PatchProcessor)).GetTypes().Where(x => x != null));

            Console.WriteLine("hello?");

            //bool ban = false;
            if (original != null && original.DeclaringType != null && types.Contains(original.DeclaringType))
            {
                throw new NullReferenceException();
            }
            return true;
